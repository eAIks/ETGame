namespace ET.Server
{
    ...
    public class C2G_GetPlayerRealmHandler : MessageSessionHandler<C2G_GetPlayerRealm, G2C_GetPlayerRealm>
    {
        protected override async ETTask Run(Session session, C2G_GetPlayerRealm request, G2C_GetPlayerRealm response)
        {
            // 验证session和player
            ...
            try
            {
                ...
                // 直接从SessionPlayerComponent获取PlayerID
                ...
                // 创建G2M消息发送到Map服务器
                G2M_GetPlayerRealm mapRequest = G2M_GetPlayerRealm.Create();
                ...
                M2G_GetPlayerRealm mapResponse = await root.GetComponent<MessageSender>().Call(mapActorId, mapRequest) as M2G_GetPlayerRealm;
                ...
                // 将Map服务器的响应转发给客户端
                response.RealmInfo = mapResponse.RealmInfo;
                response.Error = ErrorCode.ERR_Success;
                
                Log.Info($"获取玩家境界成功: PlayerId={playerID}, 境界={response.RealmInfo.MajorRealmName}-{response.RealmInfo.MinorRealmName}");
            }
            ...
            {
                Log.Error($"获取玩家境界信息失败: {e.Message}");
